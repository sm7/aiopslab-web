...
4. **Secrets exposure (env vars, logs, CI, prompt traces)**
   ...
   That combination increases the chance that secrets end up in places they don’t belong. In early 2026 reporting, researchers noted infostealers harvesting agent configuration files containing API keys and authentication tokens from popular agent setups.[^1]
...
- **Recon & Targeting**
  ...
  Identify which teams run agents, which connectors they use, where skills are sourced. Example: scan public repos/docs for “agent config” patterns.
...
- **Capability Acquisition**
  ...
  Convince agent/user to install a skill/plugin, grant a connector, or enable a tool. Example: “Install X integration to complete this request.”
